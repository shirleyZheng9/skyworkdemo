...
/**
 * TagKey MyBatis Mapper接口 - 使用PO层更新
 */

public interface TagKeyMapper {

  /**
   * 根据spaceId和id查询
   */
  TagKeyEntity selectBySpaceIdAndId(@Param("spaceId") Long spaceId, @Param("id") Long id);

  /**
   * 根据条件查询
   */
  List<TagKeyEntity> selectByCondition(Map<String, Object> params);
   ...
   * 批量插入
   */
  int batchInsert(@Param("list") List<TagKeyEntity> tagKeys);
  ...
  int updateByCondition(@Param("spaceId") Long spaceId,
                        @Param("id") Long id,
                        @Param("updateEntity") TagKeyEntity updateEntity);
