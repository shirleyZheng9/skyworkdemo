...
import com.iwhalecloud.bote.dto.plugin.params.LetterVerificationCodeGeneratorPluginParams;
 ...
 */
@Component
public class LetterVerificationCodeGeneratorPlugin extends AbstractPlugin<LetterVerificationCodeGeneratorPluginParams> {
  ...
  public LetterVerificationCodeGeneratorPlugin() {
    super(LetterVerificationCodeGeneratorPluginParams.class);
  }

  @Override
  public String getPluginCode() {
    return PluginConsts.PLUGIN_CODE_LETTER_VERIFICATION_CODE_GENERATOR;
  }
  ...
  @Override
  public void validateParams(LetterVerificationCodeGeneratorPluginParams params) {
  ...
  @Override
  public Object doRun(LetterVerificationCodeGeneratorPluginParams pluginParams) {
    try {
      // 获取参数
      ...
      for (int i = 0; i < length; i++) {
        // 从字母池中随机选择一个字母
        int index = SECURE_RANDOM.nextInt(letterPool.length());
        codeBuilder.append(letterPool.charAt(index));
      }
      ...
      result.put("message", "生成字母验证码时发生异常: " + e.getMessage());
