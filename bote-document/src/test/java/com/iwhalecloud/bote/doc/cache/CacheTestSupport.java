...
/**
 * 缓存测试公共工具：反射注入/关闭 {@code BaseSecondaryCache} 后端、构造与生产一致的 ObjectMapper。
 *
 * <p>BaseSecondaryCache 的 cacheClient/localCache/useDistributionCache/useLocalCache/useKeySetManager
 * 等字段由 Spring 在 afterPropertiesSet 中注入，纯单测中需以反射设置；关闭后端标志后 get/put 路径
 * 仅经子类 load()/loadByKey() 加载，避开 Redis/本地缓存/keySetManager。</p>
 */
