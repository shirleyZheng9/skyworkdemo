...
import com.iwhalecloud.bote.doc.common.mybatis.parser.cache.JsqlParseCache;
 ...
 * @since 2023-08-05
 */
public final class JsqlParserGlobal {
   ...
   * @since 3.5.6
   */
  public static final ExecutorService executorService = new ThreadPoolExecutor(DEFAULT_THREAD_SIZE, DEFAULT_THREAD_SIZE,
    0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<>(), r -> {
    Thread thread = new Thread(r);
    thread.setName("mybatis-plus-jsqlParser-" + thread.threadId());
    thread.setDaemon(true);
    return thread;
  });

  private JsqlParserGlobal() {
  }
