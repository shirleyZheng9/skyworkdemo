 ...
 */
@RestController
@RequestMapping(path = DocBaseConsts.API_PREFIX + "dc/attr", produces = MediaType.APPLICATION_JSON_VALUE)
@RequiredArgsConstructor
@Tag(name = "基础：属性管理")
public class DocAttrManageController {

  private final AttrSpecCache attrSpecCache;

  private final IDcCfgQueryService dcCfgQueryService;
  ...
  public ResultVO<Map<String, List<SimpleAttrDTO>>> batchGetRuleStaticAttr(@RequestBody AttrSpecQueryParams params) {
    ...
    // 并发查询
    Map<String, List<SimpleAttrDTO>> result = params.getAttrCodes().stream().distinct().parallel()
  ...
  public ResultVO<List<DcCfgDTO>> querySwitchList() {
    return ResultVO.success(dcCfgQueryService.querySwitchList());
  }
}
